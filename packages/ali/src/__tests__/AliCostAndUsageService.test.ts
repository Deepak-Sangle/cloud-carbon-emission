...
describe("Ali Cost And Usage Service", () => {
  ...
  it("Returns estimates for Compute", async () => {
    ...
    const aliCostAndUsageService = new AliCostAndUsageService(
      ...
      new StorageEstimator(ALI_CLOUD_CONSTANTS.SSDCOEFFICIENT ?? 0),
      new StorageEstimator(ALI_CLOUD_CONSTANTS.HDDCOEFFICIENT ?? 0),
      new NetworkingEstimator(ALI_CLOUD_CONSTANTS.NETWORKING_COEFFICIENT ?? 0),
      new MemoryEstimator(ALI_CLOUD_CONSTANTS.MEMORY_COEFFICIENT ?? 0),
      new UnknownEstimator(
        ALI_CLOUD_CONSTANTS.ESTIMATE_UNKNOWN_USAGE_BY ??
          EstimateUnknownUsageBy.COST
      ),
      new EmbodiedEmissionsEstimator(
        ALI_CLOUD_CONSTANTS.SERVER_EXPECTED_LIFESPAN ?? 0
      )
    );
